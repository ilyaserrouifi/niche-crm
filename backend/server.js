...
app.post('/api/leads', async (req, res) => {
    const { company_name, contact_email, niche, estimated_value, stage } = req.body;
    try {
        const result = await pool.query(
            `INSERT INTO leads (company_name, contact_email, niche, estimated_value, stage) 
             VALUES ($1, $2, $3, $4, $5) RETURNING *`,
            [company_name, contact_email, niche, estimated_value, stage || 'LEAD']
        );
        res.json(result.rows[0]);
    } catch (error) {
        res.status(500).json({ message: error.message });
...
// ============================================================
// COLD CALLERS, OUTREACHERS, FREELANCERS ROUTES
// ============================================================
