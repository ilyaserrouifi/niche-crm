...
app.get('/api/staffing/requests', authenticateToken, async (req, res) => {
    try {
        ...
        res.json(result.rows.map((r) => ({ id: String(r.id), companyName: r.company_name, skill: r.skill, description: r.description, budget: Number(r.budget || 0), timeline: r.timeline, status: r.status || 'Open', createdAt: r.created_at })));
...
app.post('/api/staffing/requests', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ id: String(r.id), companyName: r.company_name, skill: r.skill, description: r.description, budget: Number(r.budget || 0), timeline: r.timeline, status: r.status, createdAt: r.created_at });
...
app.get('/api/staffing/placements', authenticateToken, async (req, res) => {
    try {
        ...
        res.json(result.rows.map((p) => ({ id: String(p.id), freelancerId: p.freelancer_id ? String(p.freelancer_id) : '', requestId: p.request_id ? String(p.request_id) : '', freelancerName: p.freelancer_name, companyName: p.company_name, budget: Number(p.budget || 0), freelancerEarn: Number(p.freelancer_earn || 0), ourProfit: Number(p.our_profit || 0), startDate: p.start_date, status: p.status || 'Active' })));
    } catch (error) { res.json([]); }
});
...
app.post('/api/staffing/placements', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ id: String(p.id), freelancerId: p.freelancer_id ? String(p.freelancer_id) : '', requestId: p.request_id ? String(p.request_id) : '', freelancerName: p.freelancer_name, companyName: p.company_name, budget: Number(p.budget || 0), freelancerEarn: Number(p.freelancer_earn || 0), ourProfit: Number(p.our_profit || 0), startDate: p.start_date, status: p.status });
    } catch (error) { res.status(500).json({ message: error.message }); }
});
...
app.get('/api/call-recording/calls/:callerId', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT c.*, l.company_name, l.contact_name FROM calls c
