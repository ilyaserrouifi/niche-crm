...
const mapTask = (row) => ({
    ...
    deliverables: row.deliverables || [],
    updates: row.task_updates || row.updates || [],
    createdAt: row.created_at || row.createdAt || null
});
...
const ensureDatabaseSchema = async () => {
    const statements = [
        ...
        `ALTER TABLE tasks ADD COLUMN IF NOT EXISTS task_updates JSONB DEFAULT '[]'::jsonb`,
...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
    try {
        const [calls, meetings, wonDeals, pipeline, revenue, expenses] = await Promise.all([
            ...
            pool.query("SELECT COALESCE(SUM(COALESCE(value, value, 0)), 0) AS total FROM leads"),
...
app.get('/api/clients', authenticateToken, async (req, res) => {
    try {
        const [clientRows, projectRows, invoiceRows] = await Promise.all([
            pool.query('SELECT id, company as company, company_name, phone, address FROM clients ORDER BY id DESC'),
...
app.get('/api/clients/:id', authenticateToken, async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT id, company as company, company_name, phone, address FROM clients WHERE id = $1', [req.params.id]);
...
// ============================================================
const getFreelancerContext = async (req) => {
    const userResult = await pool.query(
        'SELECT id, full_name, email, role, skills, rating, status FROM users WHERE id = $1',
        [req.userId]
    );
    if (!userResult.rows.length) return null;
    const user = userResult.rows[0];
    const requestedId = req.query.freelancerId ? parseInt(req.query.freelancerId, 10) : null;
    if (user.role === 'admin' && requestedId) {
        const target = await pool.query(
            "SELECT id, full_name, email, role, skills, rating, status FROM users WHERE id = $1 AND role = 'freelancer'",
            [requestedId]
        );
        return target.rows.length ? { viewer: user, freelancer: target.rows[0], freelancerId: target.rows[0].id } : null;
    }
    if (user.role === 'freelancer') {
        return { viewer: user, freelancer: user, freelancerId: user.id };
    }
    if (user.role === 'admin') {
        return { viewer: user, freelancer: user, freelancerId: user.id };
    }
...
const assertTaskAccess = async (taskId, freelancerId, isAdmin) => {
    const result = await pool.query('SELECT * FROM tasks WHERE id = $1', [taskId]);
    if (!result.rows.length) return { error: 'not_found' };
    const task = result.rows[0];
    if (!isAdmin && String(task.assigned_to) !== String(freelancerId)) {
        return { error: 'forbidden' };
    }
    return { task };
};

app.get('/api/freelancer/dashboard', authenticateToken, async (req, res) => {
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });

        const { freelancerId, freelancer, viewer } = ctx;
        const stats = await pool.query(`
            SELECT
                COUNT(*)::int AS total,
                COUNT(*) FILTER (WHERE status ILIKE '%progress%')::int AS in_progress,
                COUNT(*) FILTER (WHERE status ILIKE '%done%' OR status ILIKE '%complete%')::int AS completed,
                COUNT(*) FILTER (WHERE due_date < CURRENT_DATE AND status NOT ILIKE '%done%')::int AS overdue
        ...
        res.json({
            ...
            stats: stats.rows[0] || { total: 0, in_progress: 0, completed: 0, overdue: 0 },
            activeTimer: activeTimer.rows[0] || null
        });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
app.get('/api/freelancer/tasks', authenticateToken, async (req, res) => {
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });

        const result = await pool.query(`
            SELECT t.*, p.project_name, c.company AS client_name, u.full_name AS assigned_to_name
            FROM tasks t
            LEFT JOIN projects p ON p.id = t.project_id
            LEFT JOIN clients c ON c.id = p.client_id
            LEFT JOIN users u ON u.id = t.assigned_to
            ...
            ORDER BY
                CASE WHEN t.due_date < CURRENT_DATE AND t.status NOT ILIKE '%done%' THEN 0 ELSE 1 END,
...
app.get('/api/freelancer/projects', authenticateToken, async (req, res) => {
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });

        const result = await pool.query(`
            SELECT DISTINCT p.*, c.company AS client_name,
                (SELECT COUNT(*)::int FROM tasks t WHERE t.project_id = p.id AND t.assigned_to = $1) AS my_tasks,
                (SELECT COUNT(*)::int FROM tasks t WHERE t.project_id = p.id AND t.assigned_to = $1 AND (t.status ILIKE '%done%' OR t.status ILIKE '%complete%')) AS my_done
            FROM projects p
            INNER JOIN tasks t ON t.project_id = p.id AND t.assigned_to = $1
            LEFT JOIN clients c ON c.id = p.client_id
            ORDER BY p.updated_at DESC
        `, [ctx.freelancerId]);

        res.json(result.rows.map((row) => ({
            ...mapProject({ ...row, client_name: row.client_name }),
...
app.put('/api/freelancer/tasks/:id/status', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const access = await assertTaskAccess(req.params.id, ctx.freelancerId, ctx.viewer.role === 'admin');
        if (access.error === 'not_found') return res.status(404).json({ message: 'Task not found' });
        if (access.error === 'forbidden') return res.status(403).json({ message: 'Not your task' });
...
app.post('/api/freelancer/tasks/:id/note', authenticateToken, async (req, res) => {
    const { text } = req.body;
    ...
    try {
        ...
        const access = await assertTaskAccess(req.params.id, ctx.freelancerId, ctx.viewer.role === 'admin');
        if (access.error) return res.status(access.error === 'forbidden' ? 403 : 404).json({ message: access.error });

        const entry = {
            id: String(Date.now()),
            user: ctx.freelancer.full_name,
            text: String(text).trim(),
            date: new Date().toISOString().slice(0, 16).replace('T', ' ')
        };
        const result = await pool.query(
            `UPDATE tasks
             SET task_updates = COALESCE(task_updates, '[]'::jsonb) || $1::jsonb, updated_at = NOW()
...
app.post('/api/freelancer/tasks/:id/deliverable', authenticateToken, async (req, res) => {
    const { name, url, note } = req.body;
    if (!name || !String(name).trim()) return res.status(400).json({ message: 'Deliverable name is required' });
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });
        ...
        const result = await pool.query(
            `UPDATE tasks
             SET deliverables = COALESCE(deliverables, '[]'::jsonb) || $1::jsonb, updated_at = NOW()
             WHERE id = $2 RETURNING *`,
            [JSON.stringify([entry]), req.params.id]
        );
        res.status(201).json(mapTask(result.rows[0]));
...
app.post('/api/freelancer/time-tracking/start', authenticateToken, async (req, res) => {
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });

        const open = await pool.query(
            'SELECT id FROM time_tracking WHERE user_id = $1 AND end_time IS NULL LIMIT 1',
            [ctx.freelancerId]
        );
        if (open.rows.length) {
            return res.status(400).json({ message: 'Timer already running', sessionId: open.rows[0].id });
        }

        const result = await pool.query(
            'INSERT INTO time_tracking (user_id, start_time) VALUES ($1, NOW()) RETURNING *',
            [ctx.freelancerId]
        );
...
app.post('/api/freelancer/time-tracking/stop', authenticateToken, async (req, res) => {
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });
        ...
        if (!result.rows.length) return res.status(404).json({ message: 'No active timer' });
...
app.get('/api/freelancer/time-tracking', authenticateToken, async (req, res) => {
    try {
        const ctx = await getFreelancerContext(req);
        if (!ctx) return res.status(403).json({ message: 'Freelancer access only' });

        const result = await pool.query(
            'SELECT * FROM time_tracking WHERE user_id = $1 ORDER BY start_time DESC LIMIT 15',
            [ctx.freelancerId]
        );
        res.json(result.rows);
    } catch (error) {
...
app.post(['/api/pipeline/deals', '/api/pipeline/leads', '/api/leads'], authenticateToken, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO leads (company, contact_name, email, contact_phone, source, niche, value, proposal_date, stage, assigned_to)
...
const mapFreelancer = (row) => ({
    ...
    specialization: row.niche || row.specialization || '',
    niche: row.niche || '',
    rate: String(row.salary ?? row.rate ?? ''),
    hourlyRate: Number(row.salary ?? row.hourlyrate ?? row.hourlyRate ?? 0),
    status: row.status || 'active',
...
app.get('/api/freelancers', authenticateToken, async (req, res) => {
    try {
        const r = await pool.query(`
            SELECT u.id, u.full_name AS name, u.full_name, u.username, u.email, u.phone, u.role, u.status,
                   u.salary, u.skills, u.niche, u.rating, u.portfolio_url, u.timezone, u.created_at,
                   (SELECT COUNT(*)::int FROM tasks t WHERE t.assigned_to = u.id) AS task_count,
                   (SELECT COUNT(*)::int FROM tasks t WHERE t.assigned_to = u.id AND t.status NOT ILIKE '%done%' AND t.status NOT ILIKE '%complete%') AS pending_tasks,
                   (SELECT COUNT(*)::int FROM tasks t WHERE t.assigned_to = u.id AND (t.status ILIKE '%done%' OR t.status ILIKE '%complete%')) AS completed_tasks,
                   (SELECT COUNT(*)::int FROM placements p WHERE p.freelancer_id = u.id) AS placement_count
            FROM users u
            WHERE u.role = 'freelancer'
            ORDER BY u.created_at DESC
        `);
        res.json(r.rows.map(mapFreelancer));
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
});

app.get('/api/freelancers/:id', authenticateToken, async (req, res) => {
    try {
        const r = await pool.query(`
            SELECT u.id, u.full_name AS name, u.full_name, u.username, u.email, u.phone, u.role, u.status,
                   u.salary, u.skills, u.niche, u.rating, u.portfolio_url, u.timezone, u.created_at,
        ...
        `, [req.params.id]);
        if (!r.rows.length) return res.status(404).json({ message: 'Freelancer not found' });
        res.json(mapFreelancer(r.rows[0]));
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
});
...
app.post('/api/freelancers', authenticateToken, async (req, res) => {
    const { name, full_name, username, email, phone, skills, specialization, niche, rate, salary, status, password, portfolio, timezone, rating } = req.body;
    ...
    try {
        ...
        const passwordHash = await bcrypt.hash(password || 'Freelancer123!', 10);
        const result = await pool.query(
            `INSERT INTO users (full_name, username, email, phone, password_hash, role, status, skills, niche, salary, rating, portfolio_url, timezone)
             VALUES ($1, $2, $3, $4, $5, 'freelancer', $6, $7, $8, $9, $10, $11, $12)
             RETURNING *`,
            [
                fullName, username || null, email, phone || null, passwordHash,
                status || 'active', skills || null, niche || specialization || null,
                Number(rate || salary || 0), Number(rating || 0), portfolio || null, timezone || null
            ]
        );
        res.status(201).json(mapFreelancer({ ...result.rows[0], name: result.rows[0].full_name }));
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
});
...
app.put('/api/freelancers/:id', authenticateToken, async (req, res) => {
    const { name, full_name, username, email, phone, skills, specialization, niche, rate, salary, status, portfolio, timezone, rating } = req.body;
    try {
        const result = await pool.query(
            `UPDATE users SET
                ...
                updated_at = NOW()
             WHERE id = $12 AND role = 'freelancer'
             RETURNING *`,
            [
                full_name || name || null, username || null, email || null, phone || null, status || null,
                skills || null, niche || specialization || null,
                rate != null || salary != null ? Number(rate ?? salary) : null,
                rating != null ? Number(rating) : null,
                portfolio || null, timezone || null,
                req.params.id
            ]
        );
        if (!result.rows.length) return res.status(404).json({ message: 'Freelancer not found' });
        res.json(mapFreelancer({ ...result.rows[0], name: result.rows[0].full_name }));
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
});
...
app.delete('/api/freelancers/:id', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query("DELETE FROM users WHERE id = $1 AND role = 'freelancer' RETURNING id", [req.params.id]);
        if (!result.rows.length) return res.status(404).json({ message: 'Freelancer not found' });
        res.json({ success: true });
    } catch (e) {
        res.status(500).json({ message: e.message });
...
app.get('/api/freelancers/:id/tasks', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
        ...
        res.json(result.rows.map((row) => ({
            ...mapTask(row),
            client: row.client_name || row.project_name || ''
        })));
    } catch (e) {
        res.status(500).json({ message: e.message });
    }
...
app.get('/api/freelancers/:id/history', authenticateToken, async (req, res) => {
    try {
        const [placements, completed] = await Promise.all([
            pool.query(`
                SELECT id, company AS client, company AS project_name, budget, freelancer_earn, status, start_date AS date, created_at
                FROM placements WHERE freelancer_id = $1 ORDER BY created_at DESC
            `, [req.params.id]),
            pool.query(`
                SELECT t.id, p.project_name, c.company AS client, t.status, t.updated_at AS date, t.priority
                FROM tasks t
                LEFT JOIN projects p ON p.id = t.project_id
                LEFT JOIN clients c ON c.id = p.client_id
                WHERE t.assigned_to = $1 AND (t.status ILIKE '%done%' OR t.status ILIKE '%complete%')
        ...
        const history = [
            ...placements.rows.map((r) => ({
                id: String(r.id),
                type: 'placement',
                projectName: r.project_name || r.company,
                client: r.client || '',
                budget: Number(r.budget || 0),
                earn: Number(r.freelancer_earn || 0),
                status: r.status,
            ...
            ...completed.rows.map((r) => ({
                id: String(r.id),
                type: 'task',
                projectName: r.project_name || 'Task',
                client: r.client || '',
                status: r.status,
                date: r.date ? new Date(r.date).toISOString().slice(0, 10) : ''
            }))
...
app.get('/api/analytics/revenue-by-niche', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT COALESCE(l.niche, 'Other') AS niche, COALESCE(SUM(COALESCE(l.value, l.value, 0)), 0)::float AS revenue
    ...
    try {
        const result = await pool.query(`
            SELECT u.full_name, u.role,
                   ...
                   COALESCE(SUM(COALESCE(l.value, l.value, 0)) FILTER (WHERE l.stage IN ('WON', 'CLOSED_WON')), 0)::float AS revenue
            FROM users u
...
app.get('/api/leads/geo', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT company AS company, contact_name AS contact, COALESCE(value, value, 0) AS value, stage, lat, lng
